...
yearsAD=[1405:5:2015];
load outputs/Tglob_new.txt
load outputs/Tglob_new_p2s.txt
load outputs/Tglob_new_m2s.txt
load outputs/Tglob_new_p3s.txt
load outputs/Tglob_new_m3s.txt

load outputs/TSH_new.txt
load outputs/TSH_new_p2s.txt
load outputs/TSH_new_m2s.txt
load outputs/TSH_new_p3s.txt
load outputs/TSH_new_m3s.txt
...
load outputs/Tglob_new_gIgcorr.txt
load outputs/Tglob_new_p2s_gIgcorr.txt
load outputs/Tglob_new_m2s_gIgcorr.txt
load outputs/Tglob_new_p3s_gIgcorr.txt
load outputs/Tglob_new_m3s_gIgcorr.txt

load outputs/TSH_new_gIgcorr.txt
load outputs/TSH_new_p2s_gIgcorr.txt
load outputs/TSH_new_m2s_gIgcorr.txt
load outputs/TSH_new_p3s_gIgcorr.txt
load outputs/TSH_new_m3s_gIgcorr.txt


mean_TGLOBAL_gIgcorr=Tglob_new_gIgcorr;
TGLOB_p2s_gIgcorr=Tglob_new_p2s_gIgcorr;
TGLOB_m2s_gIgcorr=Tglob_new_m2s_gIgcorr;

TGLOB_p3s_gIgcorr=Tglob_new_p3s_gIgcorr;
TGLOB_m3s_gIgcorr=Tglob_new_m3s_gIgcorr;
 
mean_TSH_gIgcorr=TSH_new_gIgcorr;
TSH_p2s_gIgcorr=TSH_new_p2s_gIgcorr;
TSH_m2s_gIgcorr=TSH_new_m2s_gIgcorr;

TSH_p3s_gIgcorr=TSH_new_p3s_gIgcorr;
TSH_m3s_gIgcorr=TSH_new_m3s_gIgcorr; 
...
%cd ~/Documents/WORK/nhbh/AbrametalPAGES2k2016_Supp2_inputdata/
...
pages2k(1:480) = pages2k(1:480)+(12.5E6)*archive_p2k_regional_reconstructions(4).data.rec(:);
pages2k(1:480) = pages2k(1:480)/( 37.9E6 + 12.5E6);
pages2k(481:502) = pages2k(481:502)/( 37.9E6 );
tpages2k=[1500:2003];
tpages2k=fliplr(tpages2k);
% now smooth with 25 year moving average:
...
hpages = plot(tpages2k(13:489),pages2k_25yr(13:489),'linewidth',0.5,'color',[0.5 0.5 0.5])
%legend(hcru,'CRUTEM4')
legend([hcru,hpages,hps04,hmean,hmean2],'CRUTEM4','PAGES 2k (2013)','Huang et al., 2000','Posterior mean \pm2 s.d.','corr Posterior mean \pm2 s.d.','location','northwest')
...
print -painters -depsc2 -r2500 plots/tas_glob_with_GIGcorr.eps
