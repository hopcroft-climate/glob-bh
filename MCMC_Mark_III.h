		  ...
		  //cerr <<"st " << st << endl;
        	...
        	if ((GST_id[st]-1) == number)
        	{
                        ...
                        old_func +=   FE_SS(W,times, Tequil[st],HeatFlow[st], noise, Synth, 
        		                              st,points, EuropeSet[st],0,1,misfit_array_new ); // Functional 
           ...
           for (int ij=0; ij<newpoints; ij++)
           {
            ...
            for (int st=0; st<nBH; st++)
            {
            	...
            	if ((GST_id[st]-1) == number)
            	{
				   ...
				   newFunc += FE_SS(Wnew,tnew, TEQnew[st],HtFnew[st], noise, Synth, st,newpoints, EuropeSet[st],MCMCit,burn,misfit_array);     // Functional 
