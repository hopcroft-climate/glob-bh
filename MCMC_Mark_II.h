		         ...
		         vector<Borehole_info> EuropeSet, Array2D<char> Labels)
{
	...
	ofstream outll_final;
		  ...
		  //cerr <<"st " << st << endl;
        	...
        	if ((GST_id[st]-1) == number)
        	{
                        ...
                        old_func +=   FE_SS(W,times, Tequil[st],HeatFlow[st], noise, Synth, 
        		                              st,points, EuropeSet[st],0,1,misfit_array ); // Functional 
      ...
      { 
			...
			<< "\t" << 100.0*double(Hchangea)/double(Hchanges)  << endl;
           ...
           for (int ij=0; ij<newpoints; ij++)
           {
            ...
            for (int st=0; st<nBH; st++)
            {
            	...
            	if ((GST_id[st]-1) == number)
            	{
				   ...
				   newFunc += FE_SS(Wnew,tnew, TEQnew[st],HtFnew[st], noise, Synth, st,newpoints, EuropeSet[st],MCMCit,burn,misfit_array_new);     // Functional 
            	   prior += Prior_SS(Wnew,tnew, newpoints);
            	}
            }
            ll = newFunc;
	     ...
	     //if (MCMCit >=(0)  )
		   ...
		   if ((GST_id[st]-1) == number)
            	   { 
		     FileNameM_here="";
		     FileNameM_here=FileNameM;
		   
		      tmpStr << st;
		      boreholel=tmpStr.str();
		      
		      //FileNameM_here.append(boreholel);
		      
		      FileNameM_here.append(".txt");
		      
		      cerr << "FileNameM "<< FileNameM_here << endl;
                      ofstream outM(FileNameM_here.c_str(),ios::out|ios::app);
	         
		     //for (int ijk =0; ijk<EuropeSet[st].length_T; ijk++)
                     //{
                
                    //   outM <<  misfit_array_new[st][ijk ] << "\t" ;
                    // }
                     outM << endl;
		     outM.close();    
                    }
